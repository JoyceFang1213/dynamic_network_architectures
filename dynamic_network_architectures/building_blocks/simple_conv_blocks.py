...
class ChannelAttentionModule(nn.Module):
    def __init__(self, channel, conv_op, nonlin, ratio=16):
        super(ChannelAttentionModule, self).__init__()
        
        conv_op_name = conv_op.__name__
        if "2d" in conv_op_name:
...
class SpatialAttentionModule(nn.Module):
    def __init__(self, conv_op):
        ...
        self.conv2d = conv_op(in_channels=2, out_channels=1, kernel_size=7, stride=1, padding=3)
        self.sigmoid = nn.Sigmoid()
...
class CBAMBlock(nn.Module):
    def __init__(self, channel, conv_op, nonlin):
        super(CBAMBlock, self).__init__()
        self.channel_attention = ChannelAttentionModule(channel, conv_op, nonlin)
...
class StackedConvBlocks(nn.Module):
    def __init__(self,
        ...
        self.convs = nn.Sequential(
            ...
            *[
                Identity() if not use_cbam else CBAMBlock(output_channels[-1], conv_op, nonlin)  
            ],
        )
